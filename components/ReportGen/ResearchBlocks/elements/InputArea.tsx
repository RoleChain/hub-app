...
const InputArea: FC<TInputAreaProps> = ({
  ...
  return (
    <form
      className="mx-auto flex w-full items-center justify-between rounded-lg border bg-white px-3 pb-2 pt-2 shadow-[2px_2px_38px_0px_rgba(0,0,0,0.25),0px_-2px_4px_0px_rgba(0,0,0,0.25)_inset,1px_2px_4px_0px_rgba(0,0,0,0.25)_inset]"
      onSubmit={(e) => {
        e.preventDefault();
        if (reset) reset();
        handleSubmit(promptValue);
    ...
    >
      <textarea
        ...
        ref={textareaRef}
        className="focus-visible::outline-0 my-1 min-h-[3em] w-full resize-none pl-5 font-light not-italic leading-[normal] text-[#1B1B16]/30 text-black outline-none focus-visible:ring-0 focus-visible:ring-offset-0 sm:text-xl"
        disabled={disabled}
        value={promptValue}
        required
        rows={2}

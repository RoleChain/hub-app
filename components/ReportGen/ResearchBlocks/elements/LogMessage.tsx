// LogMessage.tsx
import Accordion from "@/components/ReportGen/Task/Accordion";
    ...
    <>
      {processedLogs.map((log, index) => {
        if (
          ...
          log.header !== "selected_images" &&
          log.header !== "scraping_images"
        ) {
          return (
            <div
              key={index}
              className="mt-3 w-full max-w-4xl rounded-lg px-4 [&:not(:last-child)]:border-b"
            >
              <p className="py-3 text-base leading-relaxed text-[#444]">
                {log.text}
              </p>
            </div>
          );
